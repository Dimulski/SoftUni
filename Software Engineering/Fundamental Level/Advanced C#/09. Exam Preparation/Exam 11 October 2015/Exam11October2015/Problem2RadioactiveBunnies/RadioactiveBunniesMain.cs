namespace Problem2RadioactiveBunnies
{
    using System;
    using System.Linq;

    class RadioactiveBunniesMain
    {
        static void Main()
        {
        ...
        private static char[][] SpreadBunnies(char[][] matrix, int rows, int cols)
        {
            ...
            for (int row = 0; row <= rows; row++)
            {
                for (int col = 0; col <= cols; col++)
                {
                    if (matrix[row][col] == 'B')
                    {
                        if (row > 0) matrix[row - 1][col] = 'B';
                        if (row < rows) matrix[row + 1][col] = 'B';
                        if (col > 0) matrix[row][col - 1] = 'B';
                        if (col < cols) matrix[row][col + 1] = 'B';
                    }
                }
            }
