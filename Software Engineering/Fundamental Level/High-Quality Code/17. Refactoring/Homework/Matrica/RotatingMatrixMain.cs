namespace RotatingWalkMatrix
{
    ...
    public class RotatingMatrixMain
    {
        ...
        private static int PerformRotationWalkInMatrix(
            int[,] matrix,
            int row,
            int col,
            int valueCounter,
            int matrixLength)
        {
            ...
            while (true)
            {
                ...
                if (isDirectionXDone || isDirectionYDone || matrix[row + directionX, col + directionY] != 0)
                {
                    FindNextDirection(
                        matrix,
                        row,
